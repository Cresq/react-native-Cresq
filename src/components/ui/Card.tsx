...
export function Card({ tone = "surface", padding = 20, radius: r, gap = 12, bordered = false, onPress, style, children, ...rest }: CardProps) {
  const { colors, radius } = useTheme();
  const bg = tone === "raised" ? colors.bg.raised : tone === "transparent" ? "transparent" : colors.bg.surface;
  const base: ViewStyle = { backgroundColor: bg, borderRadius: r ?? radius.card, padding, gap, borderWidth: bordered ? 1 : 0, borderColor: colors.border.subtle };
  if (onPress) {
    return (
